    ...
    public class VehicleService : IDynamicMetaObjectProvider
    {
        ...
        public bool ProcessBlockReqByReserveModule(BCFApplication bcfApp, AVEHICLE request_block_vh, string req_block_id)
        {
            ...
            else
            {
                ...
                bool check_hid_zone_is_enough = tryCheckHIDBlockIsEnough(request_block_vh, req_block_id);
                if (!check_hid_zone_is_enough)
                {
                    LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleService), Device: DEVICE_NAME_OHx,
                       Data: $"Vh:{request_block_vh.VEHICLE_ID} ask hid entry section:{req_block_id},but this hid zone is not enough!",
                       VehicleID: request_block_vh.VEHICLE_ID,
                       CarrierID: request_block_vh.CST_ID);
                    return false;
                }
        ...
        private bool cancelBlockRequest(string cancelBlockVhID, string blockID)
        {
            ...
            bool is_success = TransferRequset(cancelBlockVhID, blockID, string.Empty, ActiveType.Override,
        ...
        public void CommandCompleteReport(string tcpipAgentName, BCFApplication bcfApp, AVEHICLE eqpt, ID_132_TRANS_COMPLETE_REPORT recive_str, int seq_num)
        {
            try
            {
                ...
                else if (recive_str.CmpStatus == CompleteStatus.CmpStatusVehicleAbort)
                {
                    if (isRetryMCSCmd)
                    {
                        scApp.ReportBLL.ReportVehicleUnassigned(finish_mcs_cmd);
                        scApp.CMDBLL.updateCMD_MCS_TranStatus(finish_mcs_cmd, E_TRAN_STATUS.Queue);
                    }
                    else if (is_direct_finish)
                        scApp.TransferService.OHT_TransferStatus(finish_ohxc_cmd, eqpt.VEHICLE_ID, ACMD_MCS.COMMAND_STATUS_BIT_INDEX_VEHICLE_ABORT);
