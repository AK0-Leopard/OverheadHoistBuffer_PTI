...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class PortStationBLL
    {
        ...
        public class DB
        {
            ...
            public bool updateEqIgnoreStatusFlag(APORTSTATION port, bool isIgnore)
            {
                try
                {
                    using (DBConnection_EF con = DBConnection_EF.GetUContext())
                    {
                        con.APORTSTATION.Attach(port);
                        port.IGNORE_STATUS_FLAG = isIgnore;
                        con.Entry(port).Property(p => p.IGNORE_STATUS_FLAG).IsModified = true;
                        portStationDao.update(con, port);
                        con.Entry(port).State = EntityState.Detached;
                    }
                }
