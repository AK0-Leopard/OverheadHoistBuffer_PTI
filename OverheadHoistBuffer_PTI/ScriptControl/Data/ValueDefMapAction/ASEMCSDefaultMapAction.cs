...
namespace com.mirle.ibg3k0.sc.Data.ValueDefMapAction
{
    public class ASEMCSDefaultMapAction : IBSEMDriver, IValueDefMapAction
    {
        ...
        private (bool isOK, string checkResult, string cstID, string boxID, string loc) checkHostCommandScan(S2F41_Scan s2F41)
        {
            ...
            string boxID = s2F41.REPITEMS.CARRIERID.CPVAL_ASCII ?? "";
        ...
        protected override void S1F3ReceiveSelectedEquipmentStatusRequest(object sender, SECSEventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    else if (s1f3.SVID[i] == SECSConst.VID_Enhanced_Carriers_For_Port_And_OHB)
                    {
                        s1f4.SV[i] = buildCarrierForPortAndOHB();
                    }
                    //else if (s1f3.SVID[i] == SECSConst.VID_CurrEq_Port_Status)
                    //{
                    //    line.EnhancedTransfersChecked = true;
        ...
        private S6F11.RPTINFO.RPTITEM.VIDITEM_51_SV buildEnhancedCarriersVIDItem()
        {
            ...
            for (int i = 0; i < cassettedata.Count; i++)
            {
                ...
                viditem_51.ENHANCED_CARRIER_INFO[i].CARRIER_ID_OBJ.CARRIER_ID = cassettedata[i].BOXID;
                viditem_51.ENHANCED_CARRIER_INFO[i].VehicleID.Crane_ID = cassettedata[i].Carrier_LOC;
                viditem_51.ENHANCED_CARRIER_INFO[i].CARRIER_LOC.CARRIER_LOC = cassettedata[i].Carrier_LOC;
        ...
        private S6F11.RPTINFO.RPTITEM.VIDITEM_55_SV buildCarrierForPortAndOHB()
        {
            S6F11.RPTINFO.RPTITEM.VIDITEM_55_SV viditem_55 = new S6F11.RPTINFO.RPTITEM.VIDITEM_55_SV();
            List<S6F11.RPTINFO.RPTITEM.ENHANCEDCARRIERINFOFORPORT> port_info_for_port_ohbs = new List<S6F11.RPTINFO.RPTITEM.ENHANCEDCARRIERINFOFORPORT>();
            List<CassetteData> cassettedata = scApp.CassetteDataBLL.loadCassetteData();
            for (int i = 0; i < cassettedata.Count; i++)
            {
                S6F11.RPTINFO.RPTITEM.ENHANCEDCARRIERINFOFORPORT info = new S6F11.RPTINFO.RPTITEM.ENHANCEDCARRIERINFOFORPORT();
                info.CarrierID = SCUtility.Trim(cassettedata[i].BOXID, true);
                info.PortID = SCUtility.Trim(cassettedata[i].Carrier_LOC, true);
                info.CarrierLoc = SCUtility.Trim(cassettedata[i].Carrier_LOC, true);
                if (scApp.TransferService.isShelfPort(info.CarrierLoc))
                {
                    // "yy/MM/dd HH:mm:ss"
                    bool is_success = DateTime.TryParseExact(cassettedata[i].CSTInDT, "yy/MM/dd HH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime parseDateTime);
                    if (is_success)
                    {
                        info.InstallTime = parseDateTime.ToString(SCAppConstants.TimestampFormat_16);
                    }
                    else
                    {
                        info.InstallTime = "";
                ...
                port_info_for_port_ohbs.Add(info);
            }
            viditem_55.ENHANCED_CARRIER_INFO_FOR_PORT_OHB = port_info_for_port_ohbs.ToArray();
            return viditem_55;
        }
        ...
        #region PTI新增
        public override bool S6F11SendTransferring(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
        ...
        public override bool S6F11SendVehicleArrived(string cmd_id, int status, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = cstLoc;
                Vids.VIDITEM_65_DVVAL_SourceID.SOURCE_ID = cmd.HOSTSOURCE;
                if (status == ACMD_MCS.COMMAND_STATUS_BIT_INDEX_LOAD_ARRIVE)
                {
                    Vids.VIDITEM_115_SV_PortID.PORT_ID = cmd.HOSTSOURCE;
                }
                else if (status == ACMD_MCS.COMMAND_STATUS_BIT_INDEX_UNLOAD_ARRIVE)
                {
                    Vids.VIDITEM_115_SV_PortID.PORT_ID = cmd.HOSTDESTINATION;
                }
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Arrived, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendVehicleAcquireStarted(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                Vids.VIDITEM_65_DVVAL_SourceID.SOURCE_ID = cmd.HOSTSOURCE;
                Vids.VIDITEM_115_SV_PortID.PORT_ID = cmd.HOSTSOURCE;
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Acquire_Started, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendVehicleAcquireCompleted(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Acquire_Completed, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendVehicleAssigned(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Assigned, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendVehicleDeparted(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Departed, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendVehicleDepositStarted(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = cstLoc;
                Vids.VIDITEM_65_DVVAL_SourceID.SOURCE_ID = cmd.HOSTSOURCE;
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Deposit_Started, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendVehicleDepositCompleted(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
                ...
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = cstLoc;
                Vids.VIDITEM_65_DVVAL_SourceID.SOURCE_ID = cmd.HOSTSOURCE;
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Deposit_Completed, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendCarrierInstalled(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
        ...
        public override bool S6F11SendCarrierRemoved(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                if (!isHostReady()) return true;
        ...
        public override bool S6F11SendVehicleUnassigned(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = cstLoc;
                Vids.VIDITEM_65_DVVAL_SourceID.SOURCE_ID = cmd.HOSTSOURCE;
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Vehicle_Unassigned, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendTransferAbortCompleted(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                Vids.VIDITEM_59_SV_CommandInfo.COMMAND_ID_OBJ.COMMAND_ID = cmd.CMD_ID;
                Vids.VIDITEM_59_SV_CommandInfo.PRIORITY_OBJ.PRIORITY = cmd.PRIORITY.ToString();
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.CARRIER_ID_OBJ.CARRIER_ID = cmd.BOX_ID;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.SOURCE_ID_OBJ.SOURCE_ID = cmd.HOSTSOURCE;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.DESTINATION_ID_OBJ.DESTINATION_ID = cmd.HOSTDESTINATION;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.CARRIER_LOC_OBJ.CARRIER_LOC = loc;
        ...
        public override bool S6F11SendTransferCompleted(ACMD_MCS cmd, CassetteData cassette, string result_code, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                Vids.VIDITEM_59_SV_CommandInfo.COMMAND_ID_OBJ = Vids.VIDITEM_58_DVVAL_CommandID;
                Vids.VIDITEM_59_SV_CommandInfo.PRIORITY_OBJ = Vids.VIDITEM_62_DVVAL_Priority;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.CARRIER_ID_OBJ.CARRIER_ID = cassette?.BOXID ?? "";
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.SOURCE_ID_OBJ.SOURCE_ID = cmd.HOSTSOURCE;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.DESTINATION_ID_OBJ.DESTINATION_ID = cmd.HOSTDESTINATION;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.CARRIER_LOC_OBJ.CARRIER_LOC = cassette?.Carrier_LOC ?? "";
        ...
        public override bool S6F11SendTransferInitiated(string cmd_id, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                string carrierLoc = cassette?.Carrier_LOC ?? "";
                string zonename = scApp.CassetteDataBLL.GetZoneName(carrierLoc);
                string cmdpriority = cmd.PRIORITY.ToString();
                Vids.VIDITEM_59_SV_CommandInfo.COMMAND_ID_OBJ.COMMAND_ID = cmdID;
                Vids.VIDITEM_59_SV_CommandInfo.PRIORITY_OBJ.PRIORITY = cmdID;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.CARRIER_ID_OBJ.CARRIER_ID = cmd.BOX_ID;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.SOURCE_ID_OBJ.SOURCE_ID = cmd.HOSTSOURCE;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.TransferInfo.DESTINATION_ID_OBJ.DESTINATION_ID = cmd.HOSTDESTINATION;
                Vids.VIDITEM_77_SV_TransferCompleteInfo.TransferInfo_.CARRIER_LOC_OBJ.CARRIER_LOC = carrierLoc;
                Vids.VIDITEM_58_DVVAL_CommandID.COMMAND_ID = cmdID;
                Vids.VIDITEM_54_DVVAL_CarrierID.CARRIER_ID = cmd.BOX_ID;
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = carrierLoc;
                Vids.VIDITEM_9999_DVVAL_CarrierZoneName.CARRIER_ZONE_NAME = zonename;
                ...
                {
                    Vids.VIDITEM_60_DVVAL_Dest.DESTINATION_ID = scApp.CassetteDataBLL.GetZoneName(cmd.HOSTDESTINATION);
                }
                else
                {
                    Vids.VIDITEM_60_DVVAL_Dest.DESTINATION_ID = cmd.HOSTDESTINATION;
        ...
        public override bool S6F11SendCarrierInstallCompleted(CassetteData cst, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                //Vids.VIDITEM_58_DVVAL_CommandID.COMMAND_ID = cst;
                Vids.VIDITEM_70_DVVAL_CraneID.Crane_ID = cst.Carrier_LOC;
        ...
        public override bool S6F11SendCarrierInstallCompletedForShelf(string carrierID, string carrierLoc, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = carrierLoc;
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Carrier_Installed_From_Shelf, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendCarrierRemovedCompletedForShelf(string carrierID, string carrierLoc, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                Vids.VIDITEM_54_DVVAL_CarrierID.CARRIER_ID = carrierID;
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = carrierLoc;
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Carrier_Removed_From_Shelf, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendCarrierIdentified(string cst_id, string box_id, App.DebugParameter.ScanReportType scanReportType, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                var cassette = scApp.CassetteDataBLL.loadCassetteDataByCstBoxID("", box_id);
                string zonename = scApp.CassetteDataBLL.GetZoneName(cassette.Carrier_LOC);
                ...
                Vids.VIDITEM_9999_DVVAL_CarrierZoneName.CARRIER_ZONE_NAME = zonename;
                Vids.VIDITEM_179_DVVAL_BOXID.BOX_ID = cassette.BOXID;
                Vids.VIDITEM_67_DVVAL_IDreadStatus.Carrier_ID_Read_Status = ((int)scanReportType).ToString();
                //scApp.CassetteDataBLL.DeleteCSTbyCstBoxID(cassette.CSTID, cassette.BOXID);

                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Carrier_Identified, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendDeviceLogicalStateMaint(List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Device_Logical_Maint, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendDeviceLogicalStateOnline(List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Device_Logical_Online, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        {
            var cassette = scApp.CassetteDataBLL.loadCassetteDataByCstBoxID(cst_id, box_id);
            return S6F11SendCarrierRemovedCompleted(cassette, reportQueues);
        }
        public override bool S6F11SendCarrierRemovedCompleted(CassetteData cassette, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                Vids.VIDITEM_70_DVVAL_CraneID.Crane_ID = crane_id;
                Vids.VIDITEM_70_SV_CraneID.Crane_ID = crane_id;
        ...
        public override bool S6F11SendCarrierRemovedFromManualPort(CassetteData cassette, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                VIDCollection Vids = new VIDCollection();
                Vids.VIDITEM_54_DVVAL_CarrierID.CARRIER_ID = cassette.BOXID;
                Vids.VIDITEM_115_SV_PortID.PORT_ID = cassette.Carrier_LOC;
                Vids.VIDITEM_56_DVVAL_CarrierLoc.CARRIER_LOC = cassette.Carrier_LOC;

                scApp.CassetteDataBLL.DeleteCSTbyBoxID(cassette.BOXID);

                AMCSREPORTQUEUE mcs_queue = S6F11BulibMessage(SECSConst.CEID_Carrier_Removed_From_MGV_Port, Vids);
                if (reportQueues == null)
                {
                    S6F11SendMessage(mcs_queue);
                }
        ...
        public override bool S6F11SendUnitAlarmSet(string unitID, string alarmID, string alarmText, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                VIDCollection Vids = new VIDCollection();
        ...
        public override bool S6F11SendCarrierIDRead(CassetteData cst, string IDreadStatus, List<AMCSREPORTQUEUE> reportQueues = null)
        {
            try
            {
                ...
                if (cst.Carrier_LOC.Contains("CR"))
                {
                    Vids.VIDITEM_70_DVVAL_CraneID.Crane_ID = cst.Carrier_LOC;
                    Vids.VIDITEM_70_SV_CraneID.Crane_ID = cst.Carrier_LOC;
                }
                else
                {
                    Vids.VIDITEM_70_DVVAL_CraneID.Crane_ID = "";
                    ...
                    for (int j = 0; j < AVIDs.Count; j++)
                    {
                        ...
                        switch (vid)
                        {
                            ...
                            case SECSConst.VID_Command_ID:
                                ...
                                break;
                            case SECSConst.VID_Command_Info:
                                vid_item = Vids.VIDITEM_59_SV_CommandInfo;
                                break;
                            case SECSConst.VID_Dest:
                                vid_item = Vids.VIDITEM_60_DVVAL_Dest;
                                break;
                            ...
                            case SECSConst.VID_TransferCompleteInfo:
                                vid_item = Vids.VIDITEM_77_SV_TransferCompleteInfo;
                                break;
                            ...
                            case SECSConst.VID_Port_ID:
                                vid_item = Vids.VIDITEM_115_SV_PortID;
                                break;
        ...
        public override bool S6F11SendMessage(AMCSREPORTQUEUE queue)
        {
            try
            {
                ...
                SXFY abortSecs = null;
                String rtnMsg = string.Empty;
                //string detail_msg = s6f11.toSECSString();
                //Console.WriteLine(detail_msg);
                ...
                //string[] dataVal;
                bool isSuccess = true;
                isSuccess = isSuccess && SECSConst.CheckDicCEIDAndRPTID(s6f11.CEID);
                if (isSuccess)
                {
                ...
                {
                    TransferServiceLogger.Info(DateTime.Now.ToString("HH:mm:ss.fff ") + "OHB >> MCS|S6F11 CEID:" + s6f11.CEID + "  No RPTID Match No VIDS");
                }
        ...
        protected override void S2F37ReceiveEnableDisableEventReport(object sender, SECSEventArgs e)
        {
            try
            {
                ...
                Boolean isEnable = s2f37.CEED[0] == 1;
